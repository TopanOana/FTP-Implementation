...
void workerThread(SOCKET ClientSocket) {
    ...
    while (!authenticated && counter < 3) {
        ...
        strncpy(current_command_word, getCommandWord(current_command).c_str(), 1024);
        current_command_word[1023] = 0;
        ...
        if (strcmp(current_command_word, USER_COMMAND) == 0) {
            if (arguments.empty()) {
                ...
                strncpy(return_val, userCommand(arguments), 1024);
                return_val[1023] = 0;
            ...
            if (arguments.empty()) {
                ...
                strncpy(return_val, passCommand(current_user, arguments), 1024);
                return_val[1023] = 0;
                if (strcmp(return_val, "false") == 0) {
    ...
    while (mode == 0) {
        ...
        if (strcmp(current_command_word, "port") == 0) {
            strncpy(command_arguments, getCommandArguments(current_command).c_str(), 1024);
            command_arguments[1023] = 0;
            //check with regex!!! <3
    ...
    strcpy(current_command, right_trim(current_command).c_str());
    ...
    while (strcmp(current_command_word, "quit") != 0) {
        ...
        if (strcmp(current_command_word, LIST_COMMAND) == 0) {
            ...
            if (strcmp(current_command, current_command_word) != 0 && arguments.empty()) {
                ...
                if (arguments.empty()) {
                    ...
                    strncpy(directory, arguments.c_str(), 499);
                    directory[499] = 0;
                }
        ...
        } else if (strcmp(current_command_word, CWD_COMMAND) == 0) {
            ...
            if (arguments.empty()) {
                ...
                if (strcmp(auxiliary, "false") == 0) {
                    ...
                    strncat(return_val, current_directory, 1024 - strlen(current_directory) - 1);
                }
        ...
        } else if (strcmp(current_command_word, RETR_COMMAND) == 0) {
            SOCKET DataSocket;
            if (mode == 1)
                DataSocket = CreateDataSocketActiveMode(ClientSocket);
            else {
        ...
        } else if (strcmp(current_command_word, STOR_COMMAND) == 0) {
            SOCKET DataSocket;
            if (mode == 1)
                DataSocket = CreateDataSocketActiveMode(ClientSocket);
            else {
